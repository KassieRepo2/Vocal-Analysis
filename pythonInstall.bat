...
REM IMPORTANT: jump past helper labels
goto :main
...
:short_to_tag
REM Converts 313 -> 3.13, 312 -> 3.12, etc. Sets PY_TAG
set "PY_TAG="
set "S=%~1"
if "%S%"=="" exit /b 1
set "MAJ=%S:~0,1%"
...
REM Prefer py launcher with proper major.minor
where py >nul 2>nul && (
  py -%PY_TAG% -V 1>nul 2>nul && ( set "PY_EXE=py -%PY_TAG%" & goto :found )
)

REM Fallback to whatever python is on PATH
...
:probe_imports
REM Returns 0 if imports OK else 1
"%VENV_PY%" -c "import importlib.util; mods=['parselmouth','numpy','matplotlib','pandas','joblib','sklearn']; import sys; sys.exit(0 if all(importlib.util.find_spec(m) for m in mods) else 1)" >nul 2>nul
exit /b %ERRORLEVEL%
...
if exist "%REQ_FILE%" (
  ...
  if errorlevel 1 (
    call :log "pip failed. If this is a Python 3.13 wheel issue, fallback to 3.12."
    exit /b 2
  )
...
call :probe_imports
if errorlevel 1 (call :log "Post-install import failed." & exit /b 2)
...
if errorlevel 2 (
  REM pip failed; rebuild using fallback python (3.12) if we were on 3.13
  call :log "Rebuilding venv with fallback Python..."
  rmdir /s /q "%VENV_DIR%" >nul 2>&1
  call :find_python %PY_SHORT_FALLBACK%
  if errorlevel 1 (
    call :log "Fallback python not found; attempting install."
    call :ensure_python || exit /b 1
    call :find_python %PY_SHORT_FALLBACK% || exit /b 1
  )
  call :ensure_venv || exit /b 1
  call :install_requirements_if_needed
)
